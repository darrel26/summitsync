<script lang="ts">
	...
	async function updateGroupAssignment(item: GroupItem, newAssigneeId: string) {
		try {
			await onUpdateGroupItem(item.id, {
...
<div class="space-y-8">
	...
	<Card.Card>
		<Card.CardHeader class="flex flex-row items-center justify-between space-y-0 pb-4">
			<div class="flex items-center gap-3">
				<div class="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-900">
					<Tent class="h-4 w-4" />
				</div>
				<div>
					<Card.CardTitle class="text-base font-bold">Shared Group Gear</Card.CardTitle>
					<Card.CardDescription>Equipment brought for the whole group</Card.CardDescription>
				</div>
			</div>
		...
		<Card.CardContent class="space-y-4">
			{#if isOwner}
				<form onsubmit={handleAddGroup} class="flex flex-col sm:flex-row gap-2">
					<Input
						type="text"
					...
					<div class="flex gap-2">
						<div class="flex items-center gap-1 rounded-md border border-slate-200 bg-white px-2.5">
						...
						<Button type="submit" disabled={addingGroup || !newGroupName.trim()} class="gap-1 shrink-0">
							<Plus class="h-4 w-4" />
			...
			{:else}
				<div class="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
					🔒 Group gear items are managed by the trip organizer. You can check off items assigned to you below.
				</div>
			{/if}

			<!-- Group Items List -->
			{#if groupItems.length === 0}
				<div class="rounded-lg border border-dashed border-slate-200 py-8 text-center text-xs text-slate-500">
					No group gear added yet. List items above and designate who will carry them.
				</div>
			{:else}
				<div class="space-y-2">
					{#each groupItems as item (item.id)}
						{@const canToggle = isOwner || item.assigned_to === currentMemberId}
						<div class="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-3 transition-colors hover:border-slate-300 {item.packed ? 'bg-slate-50/60' : ''}">
							<div class="flex flex-1 flex-wrap items-center gap-3">
								<Checkbox
									checked={!!item.packed}
									disabled={!canToggle}
									onCheckedChange={() => toggleGroupPacked(item)}
									title={canToggle ? 'Toggle packed status' : 'Only assigned member or organizer can toggle'}
								/>

								{#if editingGroupId === item.id}
									<div class="flex flex-1 flex-wrap items-center gap-2">
										<Input bind:value={editGroupName} class="h-8 flex-1 text-xs" placeholder="Item name" />
										<Input type="number" min="1" bind:value={editGroupQty} class="h-8 w-16 text-xs text-center" />
										<select
										...
										<Button size="sm" class="h-8 w-8 p-0" onclick={() => saveEditGroup(item.id)}>
											<Check class="h-3.5 w-3.5" />
										</Button>
										<Button variant="ghost" size="sm" class="h-8 w-8 p-0" onclick={() => (editingGroupId = null)}>
											<X class="h-3.5 w-3.5" />
										</Button>
									</div>
								{:else}
									<div class="flex items-center gap-2">
										<span class="text-sm font-semibold {item.packed ? 'line-through text-slate-400' : 'text-slate-900'}">
											{item.name}
										</span>
									...
									<div class="ml-auto mr-3">
										{#if isOwner}
											<select
												...
												onchange={(e: any) => updateGroupAssignment(item, e.target.value)}
												class="h-7 rounded-md border border-slate-200 bg-slate-50 px-2 text-xs text-slate-700 outline-none hover:bg-slate-100 {item.assigned_to ? 'font-semibold text-emerald-800 bg-emerald-50 border-emerald-200' : ''}"
											>
												<option value="">Unassigned</option>
												{#each members as member}
													<option value={member.id}>{member.name}</option>
										...
										{:else}
											...
											<Badge variant={item.assigned_to ? 'success' : 'secondary'} class="text-[11px]">
							...
							{#if isOwner && editingGroupId !== item.id}
								<div class="flex items-center gap-1">
									<Button variant="ghost" size="sm" class="h-8 w-8 p-0 text-slate-500 hover:text-slate-900" onclick={() => startEditGroup(item)}>
										<Pencil class="h-3.5 w-3.5" />
									</Button>
									<Button variant="ghost" size="sm" class="h-8 w-8 p-0 text-slate-500 hover:text-red-600" onclick={() => handleDeleteGroup(item)}>
	...
	<div class="space-y-4">
		<div class="flex items-center gap-3">
			...
			<div>
				<h2 class="text-base font-bold text-slate-900">Personal Packing Lists</h2>
				<p class="text-xs text-slate-500">Each member's personal gear checklist</p>
			</div>
		</div>

		{#if members.length === 0}
			<Card.Card class="py-8 text-center text-xs text-slate-500">
				No members registered yet. Add participants in the <strong>Members</strong> tab first.
			</Card.Card>
		{:else}
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{#each orderedMembers as member (member.id)}
					...
					<Card.Card class={isSelf ? 'border-emerald-300 shadow-sm' : ''}>
						<Card.CardHeader class="flex flex-row items-center justify-between space-y-0 p-4 pb-2">
							<div class="flex items-center gap-2">
								<div class="flex h-6 w-6 items-center justify-center rounded-full {isSelf ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700'} text-xs font-bold">
									{member.name.charAt(0).toUpperCase()}
								</div>
								<span class="text-sm font-bold text-slate-900">{member.name}</span>
								{#if isSelf}
									<Badge variant="success" class="text-[10px] px-1.5 py-0">You</Badge>
								{/if}
							</div>
							<span class="text-xs font-semibold text-slate-500">{packedCount}/{memberItems.length}</span>
						</Card.CardHeader>

						<Card.CardContent class="p-4 pt-2 space-y-3">
							<form onsubmit={(e) => handleAddPersonal(member.id, e)} class="flex gap-1.5">
								<Input
									placeholder="Add personal item..."
									bind:value={newPersonalNames[member.id]}
									class="h-8 text-xs"
								/>
								<Button type="submit" size="sm" class="h-8 px-2.5 shrink-0" disabled={addingPersonal[member.id] || !(newPersonalNames[member.id] || '').trim()}>
									<Plus class="h-3.5 w-3.5" />
								</Button>
							</form>

							{#if memberItems.length === 0}
								<p class="py-3 text-center text-xs text-slate-400">No personal items yet</p>
							{:else}
								<div class="space-y-1.5 max-h-48 overflow-y-auto">
									{#each memberItems as pItem (pItem.id)}
										<div class="flex items-center justify-between rounded-md bg-slate-50 p-2 text-xs">
											<div class="flex items-center gap-2 flex-1">
												<Checkbox
													checked={!!pItem.packed}
													onCheckedChange={() => togglePersonalPacked(pItem)}
												/>
												{#if editingPersonalId === pItem.id}
													<div class="flex items-center gap-1 flex-1">
														<Input bind:value={editPersonalName} class="h-7 text-xs flex-1" />
														<Button size="sm" class="h-7 w-7 p-0" onclick={() => saveEditPersonal(pItem.id)}>
															<Check class="h-3 w-3" />
														</Button>
														<Button variant="ghost" size="sm" class="h-7 w-7 p-0" onclick={() => (editingPersonalId = null)}>
															<X class="h-3 w-3" />
														</Button>
													</div>
												{:else}
													<span class="font-medium {pItem.packed ? 'line-through text-slate-400' : 'text-slate-800'}">
														{pItem.name}
													</span>
												{/if}
											</div>
											...
											{#if editingPersonalId !== pItem.id}
												<div class="flex items-center gap-0.5">
													<Button variant="ghost" size="sm" class="h-6 w-6 p-0 text-slate-400 hover:text-slate-800" onclick={() => startEditPersonal(pItem)}>
														<Pencil class="h-3 w-3" />
													</Button>
													<Button variant="ghost" size="sm" class="h-6 w-6 p-0 text-slate-400 hover:text-red-600" onclick={() => handleDeletePersonal(pItem)}>
