<script lang="ts">
	...
	async function handleRemove(member: Member) {
		if (
			confirm(
				`Remove "${member.name}" from this trip? Personal gear assigned to this member will also be deleted.`,
			)
		) {
			try {
...
<div class="space-y-6">
	...
	<Card.Card>
		<Card.CardHeader class="pb-3">
			<Card.CardTitle class="text-base font-bold"
				>Add Member</Card.CardTitle
			>
			<Card.CardDescription
				>Add friends directly or invite them via the share link.</Card.CardDescription
			>
		</Card.CardHeader>
	...
	<Card.Card>
		...
		>
			<div>
				<Card.CardTitle class="text-base font-bold"
					>Trip Participants</Card.CardTitle
				>
				<Card.CardDescription
					>People collaborating on this trip</Card.CardDescription
				>
			</div>
		...
		<Card.CardContent>
			{#if members.length === 0}
				<div class="py-8 text-center text-xs text-slate-500">
					No members registered yet. Share the trip link or add
					members above.
				</div>
			{:else}
				<div
				...
				>
					{#each members as member (member.id)}
						...
						<div
							class="flex items-center justify-between rounded-lg border p-3 transition-colors {isSelf
								? 'border-emerald-300 bg-emerald-50/40'
								: 'border-slate-200 bg-white hover:border-slate-300'}"
						>
							<div class="flex items-center gap-2.5 min-w-0">
								<div
									class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold {isSelf
								...
								>
									...
									{#if isMemberOwner}
										<Badge
											variant="warning"
											class="gap-0.5 text-[10px] px-1.5 py-0"
										>
