<script lang="ts">
	...
	async function handleAddStop(e: SubmitEvent) {
		...
		adding = true;
		try {
			const maxSort = sortedStops.reduce(
...
<div class="space-y-6">
		...
		<Card.Card>
			<Card.CardHeader class="pb-3">
				<Card.CardTitle class="text-base font-bold"
					>Add Waypoint / Checkpoint</Card.CardTitle
				>
				<Card.CardDescription
					>Meeting points, trailheads, shelters, and campsites.</Card.CardDescription
				>
			</Card.CardHeader>
	...
	{:else}
		...
		>
			🔒 View-Only Mode — Route waypoints and stops are maintained by the
			trip organizer.
		</div>
	{/if}
	...
	<Card.Card>
		...
		>
			<div>
				<Card.CardTitle class="text-base font-bold"
					>Route Trail & Stops</Card.CardTitle
				>
				<Card.CardDescription
					>Step-by-step route order</Card.CardDescription
				>
			</div>
		...
		<Card.CardContent>
			{#if sortedStops.length === 0}
				<div class="py-8 text-center text-xs text-slate-500">
					No route stops added yet. Add your starting point,
					checkpoints, and destination above.
				</div>
			{:else}
					...
					{#each sortedStops as stop, index (stop.id)}
						<div class="flex gap-3">
							...
							<div class="flex flex-col items-center">
								<div
									class="flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white shadow-sm"
								>
									{index + 1}
								</div>
								{#if index < sortedStops.length - 1}
									<div
										class="w-0.5 flex-1 bg-slate-200 my-1"
									></div>
								{/if}
							</div>
							...
							<!-- Stop Body -->
							<div
								class="flex-1 rounded-lg border border-slate-200 bg-white p-3 shadow-xs"
							>
